...
export default function ReviewsScreen() {
  ...
  // Filter reviews using product id
  ...
  const fetchReviewStats = async () => {
    // Update to use product.id if your backend supports that
  ...
  // Compute total reviews and average rating from reviewStats
  const totalReviews = reviewStats.reduce((sum, item) => sum + item.count, 0);
  const averageRating =
    totalReviews > 0
      ? reviewStats.reduce((sum, item) => sum + item.rating * item.count, 0) /
    ...
    <Container>
      ...
      <ScrollView className="px-5 mt-3">
        ...
        <View className="flex flex-row space-x-2 items-center mt-6">
          ...
          <Text fontSize="text-md" fontWeight="font-bold" className={`${isDark ? "#FFFFFFB2" : "#000000B2"}`}>
            {averageRating ? averageRating.toFixed(1) : "0"}
          </Text>
          <Text fontSize="text-md" fontWeight="font-bold" className={`ml-2 ${isDark ? "#FFFFFFB2" : "#000000B2"}`}>
            ∙ {totalReviews === 0 ? "No reviews yet" : `${totalReviews} reviews`}
          </Text>
        </View>
        ...
        {reviewStats.map((item, index) => (
          <View key={index} className="flex flex-row items-center mt-2">
            <Text className="mr-2 w-[5%]" style={{ color: isDark ? "#FFFFFF80" : "#00000080" }}>
              {item.rating}
            </Text>
            <StarFilled color={isDark ? "white" : "black"} size={16} />
            <View
              className={`flex-1 h-2 ${isDark ? "bg-[#292929]" : "bg-[#e6e6e6]"} ml-2`}
              style={{ borderRadius: 9999 }}
            >
              <View
                className={`h-full ${isDark ? "bg-white" : "bg-black"}`}
            ...
            <Text className="ml-2 w-8 text-right" style={{ color: isDark ? "#FFFFFF80" : "#00000080" }}>
              {item.count}
            </Text>
          </View>
        ))}
