    ...
    >
      ...
      >
        <View className="flex-row justify-between items-center">
          ...
          <ChevronDownIcon color={isDarkMode ? "#fff" : "#000"} size={20} />
        </View>
      </TouchableOpacity>
...
interface BusinessFormProps {
  ...
  isDarkMode: boolean;
  setError: React.Dispatch<React.SetStateAction<ErrorState>>;
  // Added DOB props for BusinessForm
  dob: {
    date: string;
    month: string;
    year: string;
...
const IndividualForm: React.FC<IndividualFormProps> = ({
  ...
  <>
    ...
    <View className="mt-4">
      ...
      {dobError && (
        <View
          className="flex flex-row items-center mt-2 mr-2 space-x-3"
...
export default function AboutYourself() {
  ...
  const validateForm = useCallback(() => {
    ...
    if (accountType === null) {
      newErrors.accountType = "Please tell us if you're an Individual or a Business";
      isValid = false;
    }
    ...
    if (accountType === "Individual" || accountType === "Business") {
      ...
      // Check if DOB fields are filled
      if (dob.date.trim() === "" || dob.month.trim() === "" || dob.year.trim() === "") {
        setDobError("Date of birth is required");
        isValid = false;
      } else {
        // If filled, calculate age for warnings or restrictions
        const age = calculateAge(dob);
        if (age !== null && accountType === "Individual") {
          if (age < 13) {
            setDobError("Users below 13 years are not allowed to use this app.");
            isValid = false;
          } else if (age < 18) {
            setDobError("Warning: Users between 13-18 years need parental guidance to use this app.");
            // Warning only, so isValid remains true
          } else {
            setDobError("");
          }
        } else {
  ...
  }, [accountType, firstName, lastName, dob, businessName, validateForm]);

  // Compute whether the form is complete to disable the continue button accordingly
        ...
        <ScrollContainer>
          <View className="flex-1 mb-[100px]">
            <View className="flex">
              ...
              <Text
                fontSize="text-base"
                className={`${isDarkMode ? "text-[#FFFFFFB2]" : "text-[#000000b2]"} mt-1`}
              >
                Spill the beans already
              </Text>
            </View>
            ...
            <View>
              <View className="mt-4">
                ...
                <View className="flex-row justify-between mt-2">
                  <TouchableOpacity
                    className={`flex-1 mr-2 p-3 rounded-[12px] h-12 border ${
                      accountType === "Individual"
                        ? "border-brand-blue"
                        : isDarkMode
                        ? "border-[#292929]"
                  ...
                  >
                    <View className="flex-row flex-1 justify-between items-center">
                      <Text fontSize="text-sm" className={isDarkMode ? "text-white" : "text-black"}>
                        Individual
                      </Text>
                      {accountType === "Individual" && (
                  ...
                  <TouchableOpacity
                    className={`flex-1 ml-2 p-3 rounded-[12px] h-12 border justify-center ${
                      accountType === "Business"
                        ? "border-brand-blue"
                        : isDarkMode
                        ? "border-[#292929]"
        ...
        <View className="py-5">
          <View className="flex-row items-center mb-4">
            <CheckBox
              ...
              onPress={() => setAgreementChecked(!agreementChecked)}
            />
            <Text className={`ml-2 ${isDarkMode ? "text-[#FFFFFFB2]" : "text-[#000000b2]"}`}>
              I agree to provide accurate and verifiable information
            </Text>
          </View>
          <Button
            variant="primary"
            onPress={handleSubmit}
            disabled={!agreementChecked || !isFormComplete}
          >
            Continue
          </Button>
        </View>
