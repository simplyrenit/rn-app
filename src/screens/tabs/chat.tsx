...
export default function Chat() {
  ...
  useFocusEffect(
    ...
    }, [authTokens, isAuthenticated, userDetails?.username, firebaseInitialized])
  );
    ...
    return (
      <StaticContainer width={100}>
        <View className="mt-4 w-full flex-1">
          <Text fontSize="text-2xl" fontWeight="font-bold" className="mb-4 px-4">
            Chat
          </Text>
    ...
    <StaticContainer width={100}>
      <View className="px-4 pb-4 mt-4 w-full">
        ...
        {isLoading ? (
          ...
          <View
            style={styles.Shadow}
            className={`border mt-6 mb-4 flex flex-row items-center h-12 rounded-xl p-2 ${isDark ? "bg-[#0F0F0F] border-[#292929]" : "bg-white border-[#E6E6E6]"}`}
          >
        ...
        {isLoading ? (
          <FlatList
            ...
            renderItem={({ item, index }) => (
              <View className="gap-2 p-3" style={{ flexDirection: 'row' }}>
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <ChatCard
                isRead={
                  item.readStatus.find((status) => status.userId === userDetails?.username)?.isRead || false
                }
                id={item.id}
                name={
                  item.initialParticipants.find((p) => p.userId !== userDetails?.username)?.username || "Unknown"
                }
                lastMessage={item.lastMessage || ""}
                unreadCount={
                  item.readCount.find((count) => count.userId === userDetails?.username)?.count || 0
                }
                lastMessageTime={item.lastMessageTime || ""}
                profilePic={
                  item.initialParticipants.find((p) => p.userId !== userDetails?.username)?.profilePicture || ""
                }
