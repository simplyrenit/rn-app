...
- [ ] P0 release gate: do not ship the new Android Firebase configuration until the existing Firestore chat data is migrated from `rn-api-35b38` to `renit-production` and the API is configured against the same destination project. Historical conversations, messages, unread state, and push-token documents must be copied, counted, and validated with a two-user physical-device chat retest before cutover. The local API currently has no active Firestore credential, so it must be brought up against the destination project only after migration parity is proven.
- [ ] P1 Flow 10: repeat sender-to-offline-recipient delivery using the rebuilt physical app. The former Expo response, `PushTicketError: Unable to retrieve the FCM server key for the recipient's app`, was addressed by moving the app to the company-owned `@simply-renit/renit` EAS project, assigning its `renit-production` FCM v1 service-account credential, replacing the Android Firebase config, and removing the hard-coded stale Expo project ID. A physical token registration, Expo ticket, and recipient-device receipt are still required before this finding can close.
