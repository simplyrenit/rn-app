...
- [ ] Flow 4 re-review retest is partially evidenced: tapping `Request review again` returned `200` and changed the authorized disposable merchant from `rejected` to `pending` in the local API. The old cached device bundle retained `rejected`; current-bundle UI readback remains blocked by the welcome-entry finding below.
...
- [ ] P1 candidate for Flow 10: the local API process reports that server-side Firestore is disabled because `FIREBASE_SECRETS_PATH` is not configured. Client-side two-user send, persistence, and read-state pass, but validate server-triggered push delivery before clearing this risk.
...
- [ ] P1 Flow 2: on the physical Android current bundle, the Welcome screen renders and JavaScript remains live (category bootstrap succeeds), but `Email`, `Google`, `Skip`, and carousel swipe controls did not react to repeated ADB touch, mouse, or motion-event injection. This blocks the current-bundle sign-in path; isolate the touch-event interception/root cause and retest each entry point before clearing it.
...
**Current confidence: 55% — not ready for production.**

Tested and passing: environment/boot (Flow 1), standard email signup plus OTP/password/logout/session recovery and location-confirmation onboarding (Flow 3 and partial Flow 5), home discovery (Flow 6), search (Flow 7), favorites (Flow 8), product-detail and owner trust paths including complete non-owner product and owner-review submission/readback (Flow 9 and Flow 15), two-user chat send, persistence, recipient receive, and read-state transition (partial Flow 10), full listing creation including gallery selection, native crop, image upload, publish, and owned-listing readback (Flow 12), owned-listing edit navigation plus Product Details, category/subcategory, and single-day unavailability update/readback (partial Flow 14), backend moderation-label migration integrity, and FAQ/navigation (partial Flow 17). The merchant re-review API transition is verified but needs fresh-device UI readback. Open findings: P0 0, P1 3 (current-bundle welcome input, server-side Firestore push configuration, and time synchronization for S3), P2 0, P3 2 (test catalogue quality and FAQ website copy). Critical evidence is still missing for current-bundle auth entry, location-denied/skip cases, merchant gating UI state, remaining seller image and inventory paths, chat push delivery, account management, the remaining support forms, and the full regression sweep. Confidence may rise only after each flow meets its exit criteria with device and log evidence; unresolved P0/P1/P2 findings keep production readiness below 100%.
