...
- [ ] P3 launch-catalogue readiness: local QA data now correctly excludes all unapproved/test listings, so discovery shows no rentable cards. Before release, load and approve genuine customer listings in each target production environment, then repeat Home, Search, product-detail, and offer checks using those records.
