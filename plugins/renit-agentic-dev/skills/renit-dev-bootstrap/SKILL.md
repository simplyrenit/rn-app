...
6. Use `scripts/validate-qa-setup.sh` as the baseline preflight. It must stop safely when QA credentials are absent and must never print their values.
7. Ask for Apple trust, OAuth, signing, or permission actions instead of attempting to bypass them.

## Output

Return the required repository handoff. Include the resolved app environment, selected device, native build path, identity checks, and every blocker. Do not expose credential values.
