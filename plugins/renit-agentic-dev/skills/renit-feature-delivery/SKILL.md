...
3. Run the narrowest relevant checks. Use `npx tsc --noEmit`, Jest, Expo config validation, or a native build only when the changed surface needs it.
4. Inspect the diff for accidental secrets, fixture data, generated artifacts, or changes outside the requested flow.
5. Return the required handoff for the quality gate. Do not self-approve a release.
