...
## Reviews written between the two QA accounts (2026-09-20, light)

Per the owner's instruction, signed in as "Development Team" and wrote one labelled review (`AGENT_QA_review ...`, product 4 stars, owner 5 stars) on the "Macbook" listing, whose owner is the "Yashwant Tejwani" account, then signed in as that account and looked at the result. The review stays on QA (I don't delete). Captures `write-review.{stars,rated}`, `all-reviews`, `owner-reviews` (`design/app/`; they show QA people, so they are untracked).

- **Write a review stars:** verified. Filled stars are black (`tone="ink"`), outline stars stay dim, the count sits at the row's end and the button changes from the bare "Submit feedback" text to the primary button once rated. The Write a review row on All reviews was reachable only through a temporary tap target on the empty "No reviews yet" line, since removed: the product page gives an empty listing no way into All reviews, so the first reviewer must come from somewhere else (a chat or booking link). Worth a product check.
- **All reviews with a review:** header score `4.0 · 1 review`, black bar for the 4-star row with a count of 1, and the review card with the reviewer's name and "a few seconds ago". Matches the frame's ink treatment.
- **Product page with a review:** the rating line shows `0.0` with five empty stars and "(1)" while the review says 4 stars: the product detail's `average_rating` had not been recomputed when the page reloaded (the All reviews screen computes 4.0 from the buckets). Likely the backend updating the average asynchronously or a stale cached product; not a layout issue, and worth a backend check.
- **Defect found and fixed: the owner profile crashed as soon as its owner had a review** (`uri.toLowerCase is not a function` from `Avatar`). The owner-review payload's reviewer `image` is an `{ image_url }` object, not the string `types.ts` claimed, and it was passed straight to `Avatar`. New `src/lib/image-url.ts` (`imageUrlOf`) normalises it, used by the owner profile and the owner's All reviews screen; `OwnerReview.reviewer.image` is typed as the union. Verified on the device: the owner profile now loads with the 5.0 rating and the owner review rail with the reviewer's photo. The owner's All reviews screen (only shown with more than one review) uses the same helper and was not opened.
- **Notifications** is still empty for both accounts: a review does not create one (notification rows come from push and chat triggers), so rows and the unread dot remain unseen.
- **Owner account, chat:** signing in as the owner shows "Unable to load chats: Error: [firestore/permission-denied]"; the Development Team account did not. A Firestore rules or claims problem for that user, not UI; noted, not investigated.

## Listing status on My Products (2026-09-20, found on the owner's physical iPhone)

Two defects on the My Products shelf, both mine:
- **"Rejected" on listings that are only pending.** The backend's `Product.admin_approved` is `BooleanField(default=False, null=True)`: every new listing starts at `False`, and it becomes `null` only when an edit sends the listing back for re-moderation (`product/serializers.py`). The API has no field that separates "not approved yet" from "disapproved" (the admin filter's "Admin Disapproved" is also `False`). `resolveListingStatus` mapped `False` to "rejected", so every fresh listing read Rejected. It now maps both `False` and `null` to "pending", and "rejected" stays in the type until the backend exposes a real rejection status. This also corrects two earlier notes in this file that called the `AGENT_QA_coverwalk` fixture "flagged Rejected by moderation": it was simply a new, unapproved listing.
- **Two tags for one state.** `MyProductCard` drew its own corner chip over the photo in addition to the pill the screen puts under the price. The card no longer draws a status chip (it keeps only the "Flagged for review" overlay, which is a different state); the pill is the one label, and the card's spoken label is derived from the same resolver.

The owner's own Product Details page shares `resolveListingStatus`, so its status pill changes the same way. Type-checked; the phone picks it up through Metro. Not re-checked against a live pending listing on the simulator (its signed-in account owns only live listings).

## Product Details hero: edge-to-edge photo (2026-09-20, from the owner's iPhone)

Deviation from Figma `1:9120`, by the owner's request after seeing real listing photos on the device. The frame contains the photo in padding on white (drawn for cut-out product shots); a real photo looked lost in it. `ProductHero` is now a full-width square photo directly below the safe area (the first version ran under the status bar and hid the product behind the Dynamic Island, so it starts below it), with the back control pinned in the header band and the favourite, count and page dots floating over the photo. The band's back control is always on screen (over the photo with the scrim until the page scrolls, then the page's own treatment), the header band fills in only as the photo's bottom edge reaches it, the status bar keeps the theme's glyphs, and the loading skeleton is the same square. `ProductImage` gained an opt-in `title` for the favourite's spoken label and its floating controls now sit on the page gutter, 8pt below the photo's top edge. Checked on the simulator at the top and scrolled; not yet re-checked on the physical device after the last change.

Photos: for the listings I could inspect (`Macbook pro`, `Macbook`), the QA API returns `images: []` with only a `cover_image`, so the hero shows one photo, exactly what it receives. `Macbook pro` was created today. Whether the post flow saved fewer photos than were chosen on the device is not established; the fixture posted from the simulator kept all three, so the backend and flow work at least there.

## Android audit, physical OnePlus CPH2423 (2026-09-21, Android 15, light and dark)

**Reference device and method.** The owner's physical phone, not an emulator (owner's call): 1080x2412 at 480 dpi = **360 dp wide**, exactly 3x, so 0.5 dp is 1.5 px. The Figma frames are 390 wide, so widths that flex (cards, grids, fields) cannot be compared 1:1; the check is the rule (gutter to gutter, no clipping, clean reflow) plus every fixed dimension (gutters, bar and row heights, type, radii, icons). The owner ruled out changing the phone's density: the app must be right on the widths users actually have, and 360 dp is the common one. The owner also relaxed the bar to "about 90% is fine", so this pass fixes visible and structural differences, not sub-dp ones. QA build (`com.renit.app.qa`, `:app:assembleQa`, arm64, QA backend), signed in as the owner's QA account, read-only: no Logout, no chat threads, no Notifications, nothing published. Captures are in `design/app/android/` (git-ignored, status bar cropped, 720 px = the 2x grid).

**Build fix needed first.** Android did not build at all: `react-native-svg` in `node_modules` had drifted to 15.15.5 (the lockfile, and Expo SDK 51, say 15.2.0) and 15.15 calls React Native APIs that 0.74 does not have. `npm ci` restored the lockfile; `pod install` then moved three iOS pods to what CI already ships (RNSVG 15.15.5 to 15.2.0, GoogleSignIn adapter and RNGoogleSignin 13.3.1 to 13.1.0) and dropped the `RNSVGFilters` resource bundle. **iOS check:** I rebuilt iOS on both svg versions and captured Search on the iPhone 16e with identical steps: 15.15.5 vs 15.2.0 differ by 0.02% of pixels (a blinking cursor). The frozen `search.nokb.light` baseline differs from both by the same 0.9%, in the same icon rows, so that drift predates the sync (the baseline is older than about 15 later commits). The sync does not move iOS.

### Screens walked (all render correctly at 360 dp unless a finding says otherwise)

| Screen | Light | Dark | Note |
| --- | --- | --- | --- |
| Home, tab bar | yes | yes | search bar, tile grid, rails, request card, five tabs |
| Search, keyboard up | yes | yes | window resizes; "Browse all" bar rides above the keyboard |
| Search results, Filters sheet | | yes | sheet sits above the nav bar; Reset and "Show 8 results" footer |
| Where sheet (places) | | yes | see F1, F2 |
| Date picker | | yes | see F5 |
| Saved, Post (step 1, 2), Chat (empty) | yes | yes | |
| Product Details, full length | yes | | edge-to-edge hero, collapsing header, map, reviews, owner card, sticky bar; see F3, F4 |
| All reviews, Owner profile | yes | | |
| Profile, Appearance sheet, FAQs, My Products | yes | yes | |
| Contact us, Who we are, Terms, Privacy, Report a problem (keyboard up) | | yes | Report button rides above the keyboard |

Not audited, with the reason: Welcome/onboarding, sign-in, phone/OTP (signed-out only, and I do not sign the owner out); Notifications and Chat threads (opening them marks things read on the real backend); Make an offer, Write a review, Edit product, Post steps 3 to 7 (they write account data; wait for `AGENT_QA_<run-id>` fixtures). The system keyboard (light IME), status bar and 3-button nav bar are platform-inherent.

### Findings

- **F1. Hardware Back on the Where sheet also leaves Search. Android only. FIXED.** One Back closed the place sheet and popped the whole Search screen (reproduced twice: sheet open, one Back, landed on Home). Cause: `search.tsx` uses a raw `@gorhom/bottom-sheet` `BottomSheet` with no back handler, so the navigator's global `hardwareBackPress` listener (`nav.tsx`) took the event. The shared `CustomBottomSheetModal` already handles it, which is why the Filters sheet was fine. Fix: a `BackHandler` listener registered while the sheet is open (9 lines, `search.tsx`); registered last so it runs first; iOS never fires that event. `auth/location.tsx` has the other raw sheet but it is persistent (`enablePanDownToClose={false}`), so no change. Type-checked (app: 0 errors; the only `tsc` errors are in `functions/`, whose dependencies are not installed here).
- **F2. The dark palette in `tailwind.config.js` lags `design-tokens.ts`. Cross-platform. NOT FIXED, needs the owner's approval.** `tailwind.config.js` dark is canvas `#0F0F0F`, surface `#1A1A1A`; the tokens moved to `#000000` and `#0F0F0F` in the Home pass. Any spot that mixes a `bg-*-dark` class with a token draws two greys: the Where-sheet search field is a visible box in a box, sampled (26,26,26) outer against (15,15,15) inner. 25 `bg-surface-dark` usages and every `bg-canvas-dark` follow the old values. Changing the config moves iOS pixels on those screens (toward the Figma), and the goal freezes the iOS baselines, so it waits for a yes.
- **F3. The Product Details map captures vertical scrolling. Both platforms, worse on Android. NOT FIXED.** `product-map.tsx:138` sets `scrollEnabled` and `zoomEnabled` true on a map that shows an "approximate area"; a swipe that starts on it pans the map, so the page seems stuck. Needs a product call (make it non-interactive, or an expand control for the interactive one).
- **F4. Copy contradicts the data. Both platforms. NOT FIXED.** "No reviews yet — be the first to rent it." and All reviews' zeroed bars sit above "See all 1 review" and a real review. Same as the average-rating recompute noted for 2026-09-20; a backend or copy fix.
- **F5. The system nav bar turns white under a dark-mode modal. Android only, cosmetic. NOT FIXED.** The date picker (and the other RN `Modal`s: address choice, chat sheets, image viewer) opens its own window, which does not inherit the per-screen `navigationBarColor`, so a white bar shows under the dark scrim. RN 0.74's `Modal` has `statusBarTranslucent` but no navigation-bar prop. The fix is to override RN's `Theme.FullScreenDialog` in `android/` (with a `values-night` variant), or to draw the picker as an in-screen overlay. Left, because it is a narrow strip and the override copies a library theme.

### dp measurement pass (2026-09-21): iOS pt against Android dp, same screens
Method: the iPhone 16e simulator and the phone, both light, both signed in as the QA account, captured at exactly 3x, then the ink bands (text lines, fields) extracted from each capture and converted to pt/dp. Both are RN layout, so a fixed dimension that differs is a platform fault; iOS is already Figma-matched. (`uiautomator` bounds give the same numbers for boxes; the bands are what let me compare text glyphs.) The simulator's `inspect` was unavailable in this session, hence the pixel bands.

| Screen | Compared | Result |
| --- | --- | --- |
| Search | field height, side gutter, section-label heights, row heights, header centring | identical: field 48, gutter 24, labels 12.67, rows equal; title centres within 0.33 |
| Profile | ten text lines from the title to "Feedback & review" | band heights identical on every line; rows below the header sit at exactly **14.0** from iOS |

The constant **14.0 dp** is the difference in safe-area top (iOS 47 pt, Android 33 dp), which is system-drawn chrome. After it, the only residue is **F6**: the two largest type sizes (the 24 pt title, and the name block) sit 1.0 to 1.3 dp higher on Android, and Search's 16 pt section labels 1 dp; 14 pt row text matches exactly. The phone's font scale is 1.0 (not the cause). It grows with type size, so it is the text engine's placement of glyphs inside the line box, not a layout difference. Platform-inherent, inside the 90% bar; a fix would have to touch the shared `Text` component for iOS's benefit and I did not fork it.

Everything else measured is inside 0.5 dp. This is two screens measured numerically, not all of them; the rest were compared visually (above). The method is cheap, so extending it to Home, Results and the static pages is mechanical.

- **F6. Large-type glyphs sit 1 to 1.3 dp higher on Android. Platform-inherent. NOT FIXED.** See above.
- **F5, proposed fix (not applied, because it only takes effect when the phone's system theme is dark and I do not change device settings to test it).** Add `android/app/src/main/res/values-night/styles.xml` redefining `Theme.FullScreenDialog` (RN's modal window theme has no parent) with RN's five items plus `<item name="android:navigationBarColor">` set to a `values-night` colour equal to the dark canvas. Follows the system theme, so an in-app Dark override on a light phone would still show the white bar. To verify: switch the phone to system dark, open the date picker.

### Addendum (2026-09-21, later): chat, and the Edit product screens

- **Chat list on Android: works, after a QA backend fix that is not in this repo.** The list showed skeletons and then the "Unable to authenticate chat" toast because `POST /api/chat/firebase-token/` returned 503: the QA web container's Google ADC had expired and the running process held the old credentials even after `gcloud auth application-default login` refreshed the file. `docker restart rn-api-web-1` on the QA host cleared it (credential refresh and `create_custom_token` both verified inside the container; the first real token request after the restart returned 200). After it, the Chat tab lists conversations with avatars, previews and timestamps. No thread was opened. Same cause and fix as `docs/known-issues.md`.
- **Edit product, walked view-only on the `Agent-qa-photos` fixture (light, nothing saved, Delete never touched):** the hub, Edit Category, Edit Product Details, Edit Product Images and Edit Unavailability render correctly at 360 dp: header, the field frames, the image grid with its dashed add tile, the calendar and the Next / "Add date log" / Update footers.
- **F7. A long unbroken title wraps mid-word in the Edit hub header. Minor, both platforms, NOT FIXED.** The header is two equal columns with a 0.4 x window-width square photo (144 dp at 360), leaving 146 dp of title; "Agent-qa-photos" (plain hyphens) is about 150 dp at 18 pt bold and breaks as "Agent-qa-ph / otos". It fits at 390 (161 dp). Any single token wider than the column breaks this way on iOS too, and real listing names have spaces, so I left the shared layout alone.

### Addendum (2026-09-21, dark sweep)

Walked in **dark** on the phone, everything previously seen only in light: Chat list, Product Details top to bottom, All reviews, About the owner, Edit product (hub, Category, Product Details, Images, Unavailability). All render correctly on the true-black canvas: surfaces, hairlines, the sheet-free screens' status and nav bars (black, light icons), the map caption, review cards and the sticky action bar. My Products and FAQs were already checked in light; Profile and the static pages in dark earlier. The in-app Appearance was set back to "Use my device settings" afterwards.

- **F8. One listing image shows upside down on Android. Data, not rendering. NOT FIXED.** On `Agent-qa-photos` the Product Details carousel's slide 2 and the Edit Product Images grid's first thumbnail (the flowers) are rotated 180 degrees; the cover, slide 1, is upright. I fetched the stored files from the QA container (read-only, nothing printed but format and size): `img0` is a plain 4032x3024 JPEG with **no EXIF orientation tag**, so its pixels are stored upside down and any viewer shows it that way; the cover is a separate 2774x3024 crop. The iPhone baseline `edit-images.light` shows the same thumbnail upright, so the fixture's image data changed after that capture (re-uploaded). Cause not proven. If it was uploaded from an Android phone, the upload path may be dropping the orientation tag without rotating the pixels, which would hit real listings; testing that needs a portrait photo picked on Android and uploaded as an `AGENT_QA` listing, which I have not done.
- **Process note:** the device dark sweep briefly left the app (Back past the tab root) and opened Chrome. No Chrome content was used, and I added a guard that checks the Renit app has focus before each step.

- **Post step 3 (the product-details form), walked on the phone in light, nothing published:** Product name is 144 px = **48 dp** (the framed-field spec), the focused field takes the brand border, the system keyboard opens below the field without covering it, and the Condition dropdown opens as an elevated card with Excellent, Good and Fair. A test string typed for the walk lived only in local wizard state and was discarded afterwards; the wizard restarts at the category list. Steps 4 to 7 still need photos and the final publish, so they wait for `AGENT_QA` fixtures (and I cannot delete a published fixture myself, which is a further reason to wait for a go-ahead).
- **Process note:** a text-matched tap on "Car" hit the wizard's "Close and discard this listing" control ("dis-car-d"), which opened the discard confirmation; I chose KEEP EDITING and nothing was lost. Matching by substring is unsafe on this app, so navigation from here uses measured coordinates.

### Home against the Figma export directly (2026-09-21, light)
`design/figma-images/home/home.light.png` (2x, 390 wide) against the phone's Home (3x, 360 wide), ink bands in the label column x 24 to 150, offsets read against the safe-area difference (Figma frame top inset 54, Android status bar 33 = 21). Tops, Figma / Android, offset: search text 84.5 / 62.0 (22.5); tile row 1 153.5 / 132.0 (21.5); its labels 213.0 / 191.7 (21.3); tile row 2 255.0 / 232.7 (22.3); its labels 329.0 / 307.7 (21.3); "Top experiences" 387.5 / 367.0 (20.5); photo card image 431.0 / 409.0 (22.0). **Every element sits within about 1 dp of the expected 21, most within 0.5, and the card image is 176 tall on both.** The search-bar and section-label glyphs show the same 1 dp text-placement residue as F6. This is one screen measured against Figma directly; the others were compared against iOS, which is Figma-matched.

### Decisions taken and fixes applied (2026-09-21, later)

Owner's rulings: tolerance is about 90%; fix F2; fixtures allowed (the owner deletes them); fix F3 only, leave F5.

- **F2 FIXED.** `tailwind.config.js` dark canvas `#0F0F0F` to `#000000` and surface `#1A1A1A` to `#0F0F0F` (the only two values that lagged; the rest of the dark block already matched). Verified on Android: the place search pill and its inner input are both (15,15,15), the box in a box is gone. **iOS:** the frozen dark Search baseline compares with **0 differing pixels** above the keyboard (its fields already came from tokens), so that baseline is unchanged. The change lands only on screens that pair a `bg-canvas-dark` / `bg-surface-dark` class with a token: `unavailability_form`, `unavailability_form_inputs`, `review-product`, `auth/location`, `chat-bubble`, `rating-filter` and the place sheet. They move one step toward the Figma's black; I checked only Search on iOS, so those iOS captures in `design/app/` are the ones that would now differ and were not re-taken.
- **F3 FIXED.** `product-map.tsx`: `scrollEnabled` and `zoomEnabled` come from `Platform.select({ android: false, default: true })` (4 lines). Verified on the phone: a swipe from the middle of the map moved the "Product's location" heading from y=1181 to y=501 px, where it used to be swallowed. iOS keeps pan and zoom.
- **F5 left open by decision.** The proposed fix stays above.
- **Process note:** the first rebuild after the tailwind change would have shipped stale colours, because NativeWind bakes the config into cached transforms and my cache-clearing `rm` silently did not run (zsh aborts a command on an unmatched glob). I stopped it, cleared the Metro cache by name, and rebuilt; the log shows "Bundler cache is empty".

### Fixture pass on Android (2026-09-21, light; the owner allowed `AGENT_QA` fixtures and will clean them up)

- **Notifications:** renders its empty state correctly ("Nothing new", bell tile, "Replies, offers and booking updates will appear here."). The account has no notifications, so rows and the unread dot are still unseen.
- **Chat thread**, on the existing `QA_CHAT_E2E_20260723` fixture conversation, view-only (nothing sent): header with avatar, name and the options button; date chip; listing card; message bubble with its time; composer with attach and offer icons and a disabled send. The options menu (View listing, Block & report in red) and the composer with the keyboard open both render correctly, with no overlap.
- **Make an offer, first step only:** the "Select a product" sheet renders correctly, but its product list is empty for this thread, so the offer and availability sheets were not reached and nothing was submitted.
- **Post steps 3 to 4, walked further:** the full step-3 form (location, landmark, price, security deposit, and a required "Me / Someone else" choice that enables Next) filled with `AGENT_QA_a1` test values; Choose Address renders correctly (Google map, pin, resolved address, Confirm, Skip for now, Use current location); step 4 "Show us how it looks" renders its empty state (dashed add tile, disabled Next). **Stopped there:** "Choose from gallery" makes the phone ask for photo access ("Allow limited access / Allow all / Don't allow"). Granting media access to the app is a persistent change on the owner's phone, so I dismissed it (one-time deny) and did not accept. Steps 5 to 7 still need that permission; the wizard's local draft was discarded. A neutral labelled test image was pushed to the phone at `Pictures/AGENT_QA_a1.jpg` for that step; it is still there (I do not delete files), so remove it by hand, or use it: choose "Allow limited access" and pick only that image.
