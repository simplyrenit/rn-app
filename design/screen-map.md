...
| Post flow | `PostSubCategories`, `AboutProduct`, `ProductImages`, `ChooseCoverImage`, `ProductAvailability`, `LocationModal`, `ReviewProduct`, `HangTight` | DONE steps 1, 2, 4 (light+dark) and the shared header; step 3 header only; steps 5–7 TODO | Section `1:13230`. Structure kept by decision: seven steps, the X exit with "Step N of 7", the app's own category list/order/names/glyphs. Header, progress segments, category and sub-category rows and the image-upload empty state match the frames to ≤0.5pt (`1:13231`, `1:13333`, `1:13650`); exports in `design/figma-images/post/light/`. Step 3 (`1:13455`) keeps its field ramp — the frame's 16pt labels, 48pt fields and hairline control edges contradict two commented decisions in `core/field.tsx` and `design-tokens.ts`, so it needs a ruling (see `design/audit.md`). Steps 5–7 could not be exported (Figwright unavailable, Figma REST 429). Captured through a temporary harness, since the Simulator cannot be tapped on this machine. Steps 4 to 7 walked on device 2026-09-20 (view-only, nothing published): step 4 Next centred, step 5 cover rewritten to the frame, step 6 month row and heading matched, step 7 inherits the Product Details components (see `design/audit.md`, "Post wizard steps 4 to 7"). |
